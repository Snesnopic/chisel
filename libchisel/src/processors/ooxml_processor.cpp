...
std::optional<ExtractedContent> OOXMLProcessor::prepare_extraction(const std::filesystem::path& input_path) {
    ...
    // opc has no stored-entry requirement, so every part gets deflated
...
std::filesystem::path OOXMLProcessor::finalize_extraction(const ExtractedContent& content, const ProcessingOptions &options) {
    ...
    if (manifest == nullptr || !write_archive_manifest(out, *manifest, get_name())) {
        Logger::log(LogLevel::Error, "Failed to finalize ooxml for file: " + content.original_path.filename().string(), get_name());
        archive_write_close(out);
        archive_write_free(out);
        cleanup_temp_dir(content.temp_dir);
        std::error_code rm_ec;
        fs::remove(tmp_path, rm_ec);
        throw std::runtime_error("OOXMLProcessor: writing entries failed");
    }
