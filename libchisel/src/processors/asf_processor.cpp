...
void AsfProcessor::recompress(const fs::path& input,
    ...
    Logger::log(LogLevel::Error, "Recompress called on AsfProcessor placeholder.", this->get_name());
...
std::optional<ExtractedContent> AsfProcessor::prepare_extraction(const fs::path& input_path) {
    Logger::log(LogLevel::Info, "ASF: Preparing cover art extraction for: " + input_path.string(), this->get_name());

    ExtractedContent content;
    content.original_path = input_path;
    content.temp_dir = make_temp_dir_for(input_path, "asf-processor");

    AudioExtractionState state = AudioMetadataUtil::extractCovers(input_path, content.temp_dir);

    if (state.extracted_covers.empty()) {
        Logger::log(LogLevel::Debug, "ASF: No embedded cover art found.", this->get_name());
...
std::filesystem::path AsfProcessor::finalize_extraction(const ExtractedContent &content, const ProcessingOptions &options) {
    Logger::log(LogLevel::Info, "ASF: Finalizing (re-inserting covers) for: " + content.original_path.string(), this->get_name());

    const AudioExtractionState* state_ptr = std::any_cast<AudioExtractionState>(&content.extras);
    if (!state_ptr) {
        Logger::log(LogLevel::Error, "ASF: Failed to retrieve extraction state.", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }
    ...
    try {
        ...
        Logger::log(LogLevel::Error, "ASF: Failed to copy media file: " + std::string(e.what()), this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }

    if (!AudioMetadataUtil::rebuildCovers(final_temp_path, *state_ptr)) {
        Logger::log(LogLevel::Error, "ASF: rebuildCovers failed", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
