 ...
 *
 * Function prototype layout (Lua 5.2):
 *   string  source name
 *   int32   line defined
 *   int32   last line defined
 *   uint8   num params
 ...
 *
 * Function prototype layout (Lua 5.3): same as 5.2 with one difference:
 *   the upvalue descriptor includes an "instack" byte and an "idx" byte.
 *   The debug section structure is identical to 5.2.
 *
 * In every version the "string" type is:
 *   5.1/5.2: uint32 length (includes NUL), then `length` bytes (or 0 = nil/empty)
 *   5.3:     uint8 length if < 0xFF, else 0xFF + uint64 length; then bytes (no NUL)
 *
...
void copy_lua_string_53(ByteReader& r, ByteWriter& w) {
    ...
    if (b == 0xFF) {
        len = r.read_u64();
        // write the 8 length bytes
        for (int i = 0; i < 8; ++i)
            w.write_u8(static_cast<uint8_t>(len >> (8*i)));
    } else {
        len = static_cast<uint64_t>(b);
    }
    w.write_bytes(r.ptr(), static_cast<size_t>(len));
    r.skip(static_cast<size_t>(len));
}

// ─── Lua 5.1 prototype parser ─────────────────────────────────────────────────

void parse_proto_51(ByteReader& r, ByteWriter& w) {
    // source name → zero it out (write empty string)
    skip_lua_string_51(r);
    w.write_u32(0); // empty source name
    ...
    for (uint32_t i = 0; i < size_const; ++i) {
        ...
        switch (tag) {
            ...
            case 3: // LUA_TNUMBER (double)
                w.write_bytes(r.ptr(), 8); r.skip(8);
                break;
            case 4: // LUA_TSTRING
                copy_lua_string_51(r, w);
                break;
            default:
...
void parse_proto_52(ByteReader& r, ByteWriter& w) {
    // source name → zero
    skip_lua_string_51(r);
    w.write_u32(0);
    ...
    for (uint32_t i = 0; i < size_const; ++i) {
        ...
        switch (tag) {
            case 0: break; // LUA_TNIL
            case 1: w.write_u8(r.read_u8()); break; // LUA_TBOOLEAN
            case 3: w.write_bytes(r.ptr(), 8); r.skip(8); break; // LUA_TNUMBER (double)
            case 4: // LUA_TSTRING (short or long, both encoded as 5.1 uint32-prefixed here)
                copy_lua_string_51(r, w);
                break;
            case 20: // LUA_TSHRSTR (Lua 5.2 short string)
                copy_lua_string_51(r, w);
                break;
            default:
...
void parse_proto_53(ByteReader& r, ByteWriter& w) {
    ...
    for (uint32_t i = 0; i < size_const; ++i) {
        ...
        switch (tag) {
            case 0:  break; // LUA_TNIL
            case 1:  w.write_u8(r.read_u8()); break; // LUA_TBOOLEAN
            case 3:  w.write_bytes(r.ptr(), 8); r.skip(8); break; // LUA_TNUMFLT (double)
            case 19: w.write_bytes(r.ptr(), 8); r.skip(8); break; // LUA_TNUMINT (int64)
            case 4:  // LUA_TSHRSTR
            case 20: // LUA_TLNGSTR
                copy_lua_string_53(r, w);
                break;
            default:
 ...
 *   5.2 — 18 bytes: magic(4) + version(1) + format(1) + LUAC_DATA(6) + int_size(1) +
 *                   size_t_size(1) + instr_size(1) + number_size(1) + integral_flag(1)
 *   5.3 — variable: magic(4) + version(1) + format(1) + LUAC_DATA(6) + int_size(1) +
...
std::vector<uint8_t> strip_debug(const std::vector<uint8_t>& in) {
    ...
    switch (version) {
        case kVer51: parse_proto_51(r, w); break;
        case kVer52: parse_proto_52(r, w); break;
        case kVer53: parse_proto_53(r, w); break;
        default:     return {};
    }
