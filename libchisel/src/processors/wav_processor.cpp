...
std::optional<ExtractedContent> WavProcessor::prepare_extraction(const fs::path& input_path) {
    Logger::log(LogLevel::Info, "WAV: Preparing cover art extraction for: " + input_path.string(), this->get_name());
    ...
    if (state.extracted_covers.empty()) {
        Logger::log(LogLevel::Debug, "WAV: No embedded cover art found.", this->get_name());
...
std::filesystem::path WavProcessor::finalize_extraction(const ExtractedContent &content, const ProcessingOptions &options) {
    Logger::log(LogLevel::Info, "WAV: Finalizing (re-inserting covers) for: " + content.original_path.string(), this->get_name());

    const AudioExtractionState* state_ptr = std::any_cast<AudioExtractionState>(&content.extras);
    if (!state_ptr) {
        Logger::log(LogLevel::Error, "WAV: Failed to retrieve extraction state.", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }
    ...
    try {
        ...
        Logger::log(LogLevel::Error, "WAV: Failed to copy audio file: " + std::string(e.what()), this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }

    if (!AudioMetadataUtil::rebuildCovers(final_temp_path, *state_ptr)) {
        Logger::log(LogLevel::Error, "WAV: rebuildCovers failed", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
