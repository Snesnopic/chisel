...
void TtaProcessor::recompress(const fs::path& input,
    ...
    Logger::log(LogLevel::Error, "Recompress called on TtaProcessor placeholder.", this->get_name());
...
std::optional<ExtractedContent> TtaProcessor::prepare_extraction(const fs::path& input_path) {
    Logger::log(LogLevel::Info, "TTA: Preparing cover art extraction for: " + input_path.string(), this->get_name());

    ExtractedContent content;
    content.original_path = input_path;
    content.temp_dir = make_temp_dir_for(input_path, "tta-processor");

    AudioExtractionState state = AudioMetadataUtil::extractCovers(input_path, content.temp_dir);

    if (state.extracted_covers.empty()) {
        Logger::log(LogLevel::Debug, "TTA: No embedded cover art found.", this->get_name());
...
std::filesystem::path TtaProcessor::finalize_extraction(const ExtractedContent &content, const ProcessingOptions &options) {
    Logger::log(LogLevel::Info, "TTA: Finalizing (re-inserting covers) for: " + content.original_path.string(), this->get_name());

    const AudioExtractionState* state_ptr = std::any_cast<AudioExtractionState>(&content.extras);
    if (!state_ptr) {
        Logger::log(LogLevel::Error, "TTA: Failed to retrieve extraction state.", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }

    const fs::path final_temp_path = fs::temp_directory_path() /
                                     (content.original_path.stem().string() + "_final" + RandomUtils::random_suffix() + ".tta");

    try {
        fs::copy_file(content.original_path, final_temp_path, fs::copy_options::overwrite_existing);
    } catch (const std::exception& e) {
        Logger::log(LogLevel::Error, "TTA: Failed to copy audio file: " + std::string(e.what()), this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
        return {};
    }

    if (!AudioMetadataUtil::rebuildCovers(final_temp_path, *state_ptr)) {
        Logger::log(LogLevel::Error, "TTA: rebuildCovers failed", this->get_name());
        cleanup_temp_dir(content.temp_dir, this->get_name());
